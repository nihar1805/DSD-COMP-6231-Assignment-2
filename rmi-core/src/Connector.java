    ...
    public void assignServer(String message) throws RemoteException, NotBoundException {
        ...
        try {
            if (message.contains("DSUM")) {
                Repository_1.obj = (IRepository) reg.lookup("r1");
//                System.out.println("In assign server obj r1");
            } else if (message.contains("ENUM KEYS")) {
                if (array[2].equals("r2")) {
                    Repository_1.obj = (IRepository) reg.lookup("r2");
                } else if (array[2].equals("r3")) {
                    Repository_1.obj = (IRepository) reg.lookup("r3");
                } else {
                    Repository_1.obj = (IRepository) reg.lookup("r1");
                }
            }
